                ...
                if url.path == '/api/files':
                    ...
                    for p in path.iterdir():
                        ...
                        try:
                            ...
                            st=p.stat(); entries.append({'name':p.name,'path':str(p.relative_to(root)),'directory':p.is_dir(),'size':st.st_size,'modified':datetime.fromtimestamp(st.st_mtime).isoformat()})
                        except (OSError,RuntimeError): continue
                    return self.output({'path':str(path.relative_to(root)) if path!=root else '', 'root':str(root), 'entries':sorted(entries,key=lambda x:(not x['directory'],x['name'].lower()))})
    ...
    def do_POST(self):
        try:
            ...
            if self.path == '/api/choose-folder':
                result=subprocess.run(['/usr/bin/osascript','-e','POSIX path of (choose folder with prompt "选择要关联的本地文件夹")'],capture_output=True,text=True,timeout=120)
                if result.returncode: return self.output({'cancelled':True})
                return self.output({'path':result.stdout.strip()})
            if self.path == '/api/open':
                _,path=linked_path(body['collection'],body['id'],body.get('path',''))
                if not path.is_dir() and path.suffix.lower() not in {'.doc','.docx','.xls','.xlsx','.csv','.tsv','.pdf','.ppt','.pptx','.txt','.md','.rtf','.png','.jpg','.jpeg','.webp','.gif','.dta','.do','.log','.bib','.tex'}: raise ValueError('此类型请在 Finder 中打开')
